...
> **Result:** ⚠️ **REVISED — PASS with open blockers.** All 41
> master-execution-prompt stages completed, **462 / 462 tests pass**,
> and the Stage 26 run executed end-to-end; **however** the post-hoc
> validity audit (EHM-CRIT-007 / EHM-HIGH-009) found the ablation
> harness never invokes the learned modules and that `dqn_core_only`'s
> apparent win is a frozen-clock artifact. The Stage 26 statistical
> claims are **withdrawn pending a corrected re-run** (see
...
| 2 | No unresolved CRITICAL audit issue | ⚠️ **BLOCKED** — EHM-CRIT-007 OPEN (ablation confound); EHM-CRIT-001..006 FIXED | ❌ |
...
**Honest framing:** ⚠️ **WITHDRAWN pending re-run (EHM-CRIT-007).**
The table above was produced by a replay runner that never invokes the
DQN/LSTM/twin/predictive/reward modules (`_select_action` returns 1
unconditionally for `enable_dqn=True`), and `dqn_core_only`
(`enable_storage=False`) never advances the simulation clock — it stays
at hour 3, `LOAD_CURVE[3] = 0.27`, the day's lowest load factor —
while `rule_based`/`full_stack` step into peak hours. The apparent
+0.614 MWh advantage is therefore a time-of-day scaling artifact, not a
controller effect, and the ablation rows `full_stack` / `no_lstm` /
`no_twin` / `no_predictive` / `no_reward` run identical policies (their
ENS spread is RNG noise from unseeded `SmartGrid` construction,
EHM-HIGH-009). These numbers must be re-derived under a corrected
harness (DQN called in `eval_mode()`; clock advancing for all policies)
